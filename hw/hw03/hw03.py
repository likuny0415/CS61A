...
def taxicab(a, b):
    ...
    """
    "*** YOUR CODE HERE ***"
    street_distance = abs(street(a) - street(b))
    avenue_distance = abs(avenue(a) - avenue(b))
...
def flatten(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    if all([type(x) == int for x in lst]):
        return lst
    if type(lst[0]) == int:
...
def balanced(m):
    ...
    if is_weight(m):
        ...
        # end is the third factor of side which is mobile or weight
        left_weight = length(left(m)) * total_weight(end(left(m)))
        right_weight = length(right(m)) * total_weight(end(right(m)))
